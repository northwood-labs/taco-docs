...
const pluginProtocolVersion = 7

var (
	_ goplugin.Plugin = &formatter{}
...
// go-plugin's Plugin interface (Server/Client factory pattern). This
// indirection lets go-plugin manage the RPC lifecycle while keeping plugin
// authors' code focused on the formatting logic itself.
...
// Name returns the formatter's name.
func (f *formatter) Name() string {
	return f.name
}

// Version returns the formatter's version.
func (f *formatter) Version() string {
	return f.version
}

// Execute runs the formatter's print function and returns the result.
func (f *formatter) Execute(args *ExecuteArgs) (string, error) {
	result, err := f.printer(args.Config, args.Module)
	if err != nil {
