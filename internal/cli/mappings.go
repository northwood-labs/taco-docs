...
// paths. This indirection exists because CLI flags use kebab-case
// (--output-file) while the config file uses a nested structure
// (output.file). Without this map, bindFlags() would need hardcoded
// knowledge of every flag's config path, making it fragile to add new flags.
// The map also serves as the canonical list of which flags are bindable —
